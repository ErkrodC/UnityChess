...
namespace UnityChess.Presentation {
	/*public class VisualPiece : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler {
		...
		public void OnDrag(PointerEventData eventData) {
			...
			Vector3 nextPiecePositionSS = new Vector3(eventData.position.x, eventData.position.y, piecePositionSS.z);
		...
		public void OnPointerUp(PointerEventData eventData) {
			...
			BoardManager.Instance.GetSquareGOsWithinRadius(potentialLandingSquares, thisTransform.position,
			...
			float shortestDistanceFromPieceSquared =
				(closestSquareTransform.transform.position - thisTransform.position).sqrMagnitude;
			for (int i = 1; i < potentialLandingSquares.Count; i++) {
				GameObject potentialLandingSquare = potentialLandingSquares[i];
				float distanceFromPieceSquared =
					(potentialLandingSquare.transform.position - thisTransform.position).sqrMagnitude;
